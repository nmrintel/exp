...
def exponentiation(A:np.ndarray,eps)->tuple[float,np.ndarray,float]:
...
def main():
    ...
    data = []
    EPS_list = [10**(-8),10**(-6),10**(-4),10**(-2)]
    for i, deg in enumerate(EPS_list):
    ...
    for idx,name in enumerate(labels):
        plt.clf() 
        plt.scatter(EPS_list,data[:,idx])
        plt.plot(EPS_list,data[:,idx])
